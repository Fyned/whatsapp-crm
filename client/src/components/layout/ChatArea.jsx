...
export default function ChatArea({ activeSession, activeContact }) {
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            const currentChatOwner = cleanId(activeContact.id || activeContact.phone_number);
  ...
  const fetchMessages = async () => {
    const contactIdClean = cleanId(activeContact.id || activeContact.phone_number);
    ...
    if (data) {
        setMessages(data);
        if (data.length > 0) {
            setOldestMessageId(data[0]._serialized || null);
        }
        setTimeout(scrollToBottom, 100);
    }
  };
  ...
  const handleFetchHistory = async () => {
      if (!targetNumber) return alert("Numara yok.");
      ...
      try {
        ...
        if (data.success) {
            alert(`${data.count} adet geçmiş mesaj getirildi.`);
      ...
      <div className="bg-gray-100 border-b shadow-sm z-10">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    <div className="font-bold text-gray-800">
                        {activeContact.push_name || activeContact.name || activeContact.phone_number}
                    </div>
        ...
        <div className="bg-white p-2 flex items-center justify-between px-4 shadow-inner">
            <div className="flex items-center gap-2 text-sm text-gray-600">
                <History size={16}/>
                <span className="font-medium hidden md:inline">Geçmiş:</span>
                <input 
        ...
        {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
                <div 
                    ...
                    `}
                >
                    <p className="text-gray-800 leading-relaxed wrap-break-word">{msg.body}</p>
